...
// Test: Make sure previous cases not broken
Insight.prototype.askPermission = function (msg, cb, options) {
	...
	var prompt = inquirer.prompt({
        ...
        // If no override specified, opt-in by default (for backwards compatibility)
		default: options && 'optInByDefault' in options ? options.optInByDefault : true
	}, function (result) {
	...
	permissionTimeout = setTimeout(function () {
		...
		// If no override specified, opt-in by default (for backwards compatibility)
		this.optOut = options && 'optInByDefault' in options ? !options.optInByDefault : false;
		cb(null, !this.optOut); 
